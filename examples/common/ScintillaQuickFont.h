...
{
    switch (family) {
    case Bundled_test_font_family::CascadiaCode:
        return QStringLiteral("Cascadia Code");
    case Bundled_test_font_family::Cousine:
        return QStringLiteral("Cousine");
    }
...
inline QString bundled_test_font_directory_name(Bundled_test_font_family family)
{
    switch (family) {
    case Bundled_test_font_family::CascadiaCode:
        return QStringLiteral("CascadiaCode");
    case Bundled_test_font_family::Cousine:
        return QStringLiteral("Cousine");
    }
...
inline QString locate_bundled_test_font_file(
    Bundled_test_font_family family,
    const QString &file_name)
{
    const QString relative_path = QStringLiteral("fonts/%1/%2")
...
inline bool load_bundled_test_font_family(
    Bundled_test_font_family family,
    QString *error_message = nullptr)
{
    ...
    for (const QString &file_name : files) {
        ...
        if (path.isEmpty()) {
            failures.push_back(QStringLiteral("%1/%2 not found")
                                   .arg(bundled_test_font_directory_name(family))
                                   .arg(file_name));
            continue;
        }
