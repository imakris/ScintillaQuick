...
void MarginView::PaintOneMargin(Surface *surface, PRectangle rc, PRectangle rcOneMargin, const MarginStyle &marginStyle,
	const EditModel &model, const ViewStyle &vs, Render_collector *collector) const {
	...
	if (visibleLine < 0) {
		...
		yposScreen += skipped_display_lines * vs.lineHeight;
		visibleLine = 0;
	}
	...
	while ((visibleLine < model.pcs->LinesDisplayed()) && yposScreen < rc.bottom) {
		...
		if (marginStyle.style == MarginType::Number) {
			if (firstSubLine) {
				...
				rcNumber.left = xpos;
				if (collector) {
					Captured_margin_text capturedText;
					capturedText.x = static_cast<float>(rcNumber.left);
					capturedText.y = static_cast<float>(rcNumber.top);
					capturedText.left = static_cast<float>(rcNumber.left);
					capturedText.top = static_cast<float>(rcNumber.top);
					capturedText.right = static_cast<float>(rcNumber.right);
					capturedText.bottom = static_cast<float>(rcNumber.bottom);
					capturedText.baseline_y = static_cast<float>(rcNumber.top + vs.maxAscent);
					capturedText.document_line = static_cast<int>(lineDoc);
					capturedText.subline_index = 0;
					capturedText.utf8_text = sNumber;
					capturedText.style_id = StyleLineNumber;
					if (collector->wants_static_content()) {
						collector->add_margin_text(capturedText);
					}
				}
		...
		if (marks) {
			...
			for (int markBit = 0; (markBit <= MarkerMax) && marksBar; markBit++) {
				if ((marksBar & 1) && (vs.markers[markBit].markType == MarkerSymbol::Bar)) {
					...
					const LineMarker::FoldPart barPart = PartForBar(markBefore, markAfter);
					vs.markers[markBit].Draw(surface, rcMarker, vs.styles[StyleLineNumber].font.get(),
						barPart, marginStyle.style);
					if (collector) {
						Captured_marker_symbol capturedMarker;
						capturedMarker.left = static_cast<float>(rcMarker.left);
						...
						capturedMarker.back_rgba = static_cast<std::uint32_t>(vs.markers[markBit].back.AsInteger());
						capturedMarker.back_rgba_selected = static_cast<std::uint32_t>(vs.markers[markBit].backSelected.AsInteger());
						capturedMarker.document_line = static_cast<int>(lineDoc);
						capturedMarker.fold_part = static_cast<int>(barPart);
						if (collector->wants_static_content()) {
			...
			for (int markBit = 0; (markBit <= MarkerMax) && marks; markBit++) {
				if ((marks & 1) && (vs.markers[markBit].markType != MarkerSymbol::Bar)) {
					...
					if (collector) {
						...
						capturedMarker.back_rgba_selected = static_cast<std::uint32_t>(vs.markers[markBit].backSelected.AsInteger());
						capturedMarker.document_line = static_cast<int>(lineDoc);
						capturedMarker.fold_part = static_cast<int>(part);
						if (collector->wants_static_content()) {
							collector->add_marker_symbol(capturedMarker);
						}
					}
...
void MarginView::PaintMargin(Surface *surface, Sci::Line topLine, PRectangle rc, PRectangle rcMargin,
	const EditModel &model, const ViewStyle &vs, Render_collector *collector) {
	...
	for (const MarginStyle &marginStyle : vs.ms) {
		if (marginStyle.width > 0) {
			...
			PaintOneMargin(surface, rc, rcOneMargin, marginStyle, model, vs, collector);
		}
	}
