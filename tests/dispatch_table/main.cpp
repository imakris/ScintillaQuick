...
void test_tracked_scroll_width_reset_table()
{
    ...
    SQ_EXPECT(scene_graph_update_request(SCI_SETTEXT).scroll_width_reset);
    SQ_EXPECT(scene_graph_update_request(SCI_CLEARALL).scroll_width_reset);
    SQ_EXPECT(scene_graph_update_request(SCI_SETWRAPMODE).scroll_width_reset);
    SQ_EXPECT(scene_graph_update_request(SCI_STYLESETFONT).scroll_width_reset);
    SQ_EXPECT(scene_graph_update_request(SCI_SETMARGINWIDTHN).scroll_width_reset);
    SQ_EXPECT(scene_graph_update_request(SCI_SETDOCPOINTER).scroll_width_reset);

    // Messages that must NOT trigger a scroll-width reset:
    SQ_EXPECT(!scene_graph_update_request(SCI_GETTEXT).scroll_width_reset);
    SQ_EXPECT(!scene_graph_update_request(SCI_GOTOPOS).scroll_width_reset);
    SQ_EXPECT(!scene_graph_update_request(SCI_SETSEL).scroll_width_reset);
    SQ_EXPECT(!scene_graph_update_request(SCI_INSERTTEXT).scroll_width_reset);
}
