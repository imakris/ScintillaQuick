...
struct Fixture_editor
{
    ...
    QString window_state_text(const char* fixture_name, const char* stage) const
    {
        ...
        state += QStringLiteral("content_size=%1x%2\n").arg(content_size.width()).arg(content_size.height());
    ...
    QImage capture_image(const char* fixture_name)
    {
        ...
        if (!wait_for_ready(fixture_name)) {
            qWarning("capture[%s]: window became unavailable after resize", fixture_name);
            return QImage();
        }
    ...
    for (int y = 0; y < h; ++y) {
        ...
        for (int x = 0; x < w; ++x) {
                ...
                db > k_channel_tolerance || da > k_channel_tolerance)
            {
                ...
                dl[x] = qRgb(qRed(al[x]) / 3, qGreen(al[x]) / 3, qBlue(al[x]) / 3);
            }
        }
    }
...
{
    return QStringLiteral(ARTIFACT_DIR) + QDir::separator() + QString::fromUtf8(name) + QStringLiteral("_")
        + QString::fromUtf8(suffix) + QStringLiteral(".png");
}

enum class Fixture_outcome
{
    pass,
    fail,
    generated
...
Fixture_outcome run_visual_fixture(const char* name, const QImage& actual)
{
    ...
    if (regenerate) {
        ...
        if (!actual.save(bp)) {
            qWarning("  [%s] FAIL: could not save baseline to %s", name, qPrintable(bp));
            return Fixture_outcome::fail;
        }
    ...
    qWarning("  [%s] FAIL: %d differing pixels (%.2f%%), artifacts saved to %s",
        name, cmp.differing_pixels, cmp.diff_ratio * 100.0, qPrintable(QStringLiteral(ARTIFACT_DIR)));
    return Fixture_outcome::fail;
}
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        if (!editor.wait_for_next_paint(previous_paint_counter)) {
            qWarning("  [%s] FAIL: timed out waiting for repaint after wheel step %d",
                scenario_name.toUtf8().constData(), step);
            return Fixture_outcome::fail;
        }
        if (current_first_visible_line == previous_first_visible_line) {
            qWarning(
                "  [%s] FAIL: wheel step %d did not change first visible line",
...
static Fixture_outcome vr_plain_ascii_short()
{
    ...
    return run_visual_fixture("plain_ascii_short", f.capture_image("plain_ascii_short"));
}
...
static Fixture_outcome vr_mixed_styles_wrap()
{
    ...
    return run_visual_fixture("mixed_styles_wrap", f.capture_image("mixed_styles_wrap"));
}
...
static Fixture_outcome vr_selection_single_line()
{
    ...
    return run_visual_fixture("selection_single_line", f.capture_image("selection_single_line"));
}
...
static Fixture_outcome vr_selection_wrap_boundary()
{
    ...
    return run_visual_fixture("selection_wrap_boundary", f.capture_image("selection_wrap_boundary"));
}
...
static Fixture_outcome vr_caret_mid_line()
{
    ...
    return run_visual_fixture("caret_mid_line", f.capture_image("caret_mid_line"));
}
...
static Fixture_outcome vr_caret_wrap_continuation()
{
    ...
    return run_visual_fixture("caret_wrap_continuation", f.capture_image("caret_wrap_continuation"));
}
...
static Fixture_outcome vr_current_line_basic()
{
    ...
    return run_visual_fixture("current_line_basic", f.capture_image("current_line_basic"));
}
...
static Fixture_outcome vr_current_line_wrap()
{
    ...
    return run_visual_fixture("current_line_wrap", f.capture_image("current_line_wrap"));
}
...
static Fixture_outcome vr_margin_numbers_basic()
{
    ...
    f.set_text("line one\nline two\nline three\nline four\nline five");
    return run_visual_fixture("margin_numbers_basic", f.capture_image("margin_numbers_basic"));
}
...
static Fixture_outcome vr_plain_indicator_basic()
{
    ...
    return run_visual_fixture("plain_indicator_basic", f.capture_image("plain_indicator_basic"));
}
...
static Fixture_outcome vr_control_repr_simple()
{
    ...
    return run_visual_fixture("control_repr_simple", f.capture_image("control_repr_simple"));
}
...
static Fixture_outcome vr_multi_selection()
{
    ...
    return run_visual_fixture("multi_selection", f.capture_image("multi_selection"));
}
...
static Fixture_outcome vr_rectangular_selection()
{
    ...
    return run_visual_fixture("rectangular_selection", f.capture_image("rectangular_selection"));
}
...
static Fixture_outcome vr_current_line_frame()
{
    ...
    f.set_text("first line\nsecond line\nthird line");
    f.editor.send(SCI_GOTOPOS, 5);
    return run_visual_fixture("current_line_frame", f.capture_image("current_line_frame"));
}
...
static Fixture_outcome vr_squiggle_indicator()
{
    ...
    return run_visual_fixture("squiggle_indicator", f.capture_image("squiggle_indicator"));
}
...
static Fixture_outcome vr_box_indicator()
{
    ...
    return run_visual_fixture("box_indicator", f.capture_image("box_indicator"));
}
...
static Fixture_outcome vr_marker_symbol()
{
    ...
    return run_visual_fixture("marker_symbol", f.capture_image("marker_symbol"));
}
...
static Fixture_outcome vr_multi_caret()
{
    ...
    return run_visual_fixture("multi_caret", f.capture_image("multi_caret"));
}
...
static Fixture_outcome vr_whitespace_visible()
{
    ...
    f.set_text("a b\tc");
    return run_visual_fixture("whitespace_visible", f.capture_image("whitespace_visible"));
}
...
static Fixture_outcome vr_eol_annotation()
{
    ...
    f.pump();
    return run_visual_fixture("eol_annotation", f.capture_image("eol_annotation"));
}
...
static Fixture_outcome vr_annotation()
{
    ...
    f.pump();
    return run_visual_fixture("annotation", f.capture_image("annotation"));
}
...
static Fixture_outcome vr_indent_guide()
{
    ...
    f.set_text("if (x) {\n    if (y) {\n        z = 1;\n    }\n}");
    return run_visual_fixture("indent_guide", f.capture_image("indent_guide"));
}
...
static Fixture_outcome vr_style_underline()
{
    ...
    f.pump();
    return run_visual_fixture("style_underline", f.capture_image("style_underline"));
}
...
{
    Fixture_editor f;
    f.editor.send(SCI_SETPROPERTY, reinterpret_cast<uptr_t>("fold"), reinterpret_cast<sptr_t>("1"));
    ...
    f.editor.send(SCI_SETAUTOMATICFOLD, SC_AUTOMATICFOLD_SHOW | SC_AUTOMATICFOLD_CHANGE, 0);
    ...
    f.pump();
    return run_visual_fixture("fold_display_text", f.capture_image("fold_display_text"));
}
...
static Fixture_outcome vr_fold_markers()
{
    ...
    f.editor.send(SCI_SETFOLDLEVEL, 3, SC_FOLDLEVELBASE);
    f.pump();
    return run_visual_fixture("fold_markers", f.capture_image("fold_markers"));
}
...
static Fixture_outcome vr_annotation_boxed()
{
    ...
    f.editor.send(SCI_ANNOTATIONSETTEXT, 0, reinterpret_cast<sptr_t>(annot_text));
    f.pump();
    return run_visual_fixture("annotation_boxed", f.capture_image("annotation_boxed"));
}
...
static Fixture_outcome vr_eol_annotation_boxed()
{
    ...
    f.editor.send(SCI_EOLANNOTATIONSETTEXT, 0, reinterpret_cast<sptr_t>(annot_text));
    f.pump();
    return run_visual_fixture("eol_annotation_boxed", f.capture_image("eol_annotation_boxed"));
}
...
int main(int argc, char** argv)
{
    ...
    if (!selected_fixture_name.isEmpty() && !ran_any_fixture) {
        qCritical("FAIL: requested fixture '%s' was not found", qPrintable(selected_fixture_name));
        return 2;
    }

    int total = g_pass_count + g_fail_count + g_gen_count;
    qDebug(
        "=== Results: %d passed, %d generated, %d failed (of %d) ===",
        g_pass_count,
        g_gen_count,
        g_fail_count,
        total);
