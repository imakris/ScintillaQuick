...
int main(int argc, char** argv)
{
    ...
    QCommandLineOption outputOption(QStringList{QStringLiteral("o"), QStringLiteral("output")},
        QStringLiteral("Write JSON results to <file>."), QStringLiteral("file"));
    QCommandLineOption scenarioOption(QStringList{QStringLiteral("s"), QStringLiteral("scenario")},
        QStringLiteral("Run only the named scenario(s)."), QStringLiteral("name"));
    ...
    if (should_run_scenario(QStringLiteral("caret_step_right_latency_64"))) {
        ...
        scenarios.append(measure_paint_latency_scenario(
            QStringLiteral("caret_step_right_latency_64"), 64, editor, paint_counter, [&](int) {
    ...
    if (should_run_scenario(QStringLiteral("caret_step_left_latency_64"))) {
        ensure_large_document_loaded();
        scenarios.append(measure_paint_latency_scenario(
            QStringLiteral("caret_step_left_latency_64"), 64, editor, paint_counter, [&](int) {
    ...
    if (should_run_scenario(QStringLiteral("vertical_scroll_step_latency_64"))) {
        ensure_large_document_loaded();
        scenarios.append(measure_paint_latency_scenario(
            QStringLiteral("vertical_scroll_step_latency_64"), 64, editor, paint_counter,
            [&](int) {
    ...
    if (should_run_scenario(QStringLiteral("vertical_scroll_page_step_latency_16"))) {
        ensure_large_document_loaded();
        scenarios.append(measure_paint_latency_scenario(
            QStringLiteral("vertical_scroll_page_step_latency_16"), 16, editor, paint_counter,
            [&](int) {
    ...
    if (should_run_scenario(QStringLiteral("vertical_scroll_bounce_latency_48"))) {
        ensure_large_document_loaded();
        scenarios.append(measure_paint_latency_scenario(
            QStringLiteral("vertical_scroll_bounce_latency_48"), 48, editor, paint_counter,
            [&](int step) {
                const int current_top_line = static_cast<int>(editor.send(SCI_GETFIRSTVISIBLELINE));
                const int delta = (step % 2 == 0) ? 3 : -3;
                const int max_top_line = std::max(0,
                    static_cast<int>(editor.send(SCI_GETLINECOUNT)) - static_cast<int>(editor.send(SCI_LINESONSCREEN)));
    ...
    if (should_run_scenario(QStringLiteral("vertical_wheel_bounce_latency_48"))) {
        ...
        scenarios.append(measure_paint_latency_scenario(
            QStringLiteral("vertical_wheel_bounce_latency_48"), 48, editor, paint_counter,
            [&](int step) {
    ...
    if (should_run_scenario(QStringLiteral("wrapped_wheel_bounce_latency_24"))) {
        ...
        const QPointF wheel_point(
            ...
            std::max<qreal>(editor.height() * 0.5, 32.0));
        scenarios.append(measure_paint_latency_scenario(
            QStringLiteral("wrapped_wheel_bounce_latency_24"), 24, editor, paint_counter,
            [&](int step) {
                const bool wheel_up = (step % 2) == 0;
                send_wheel_event(
                    window,
    ...
    if (should_run_scenario(QStringLiteral("scroll_after_edit_latency_32"))) {
        ensure_large_document_loaded();
        scenarios.append(measure_paint_latency_scenario(
            QStringLiteral("scroll_after_edit_latency_32"), 32, editor, paint_counter, [&](int) {
                const sptr_t target_line =
                    std::min<sptr_t>(editor.send(SCI_GETLINECOUNT) - 1, editor.send(SCI_GETFIRSTVISIBLELINE) + 5);
    ...
    if (should_run_scenario(QStringLiteral("selection_drag_latency_48"))) {
        ...
        scenarios.append(measure_paint_latency_scenario(
            QStringLiteral("selection_drag_latency_48"), 48, editor, paint_counter, [&](int index) {
                ...
                send_mouse_event(window, editor, QEvent::MouseMove, drag_point, Qt::NoButton, Qt::LeftButton);
            }));
        const QPointF drag_end_point = local_point_for_position(editor, drag_start + 48);
        send_mouse_event(window, editor, QEvent::MouseButtonRelease, drag_end_point, Qt::LeftButton, Qt::NoButton);
        pump_gui(4);
    }
    ...
    if (should_run_scenario(QStringLiteral("zoom_wheel_bounce_latency_24"))) {
        ...
        scenarios.append(measure_paint_latency_scenario(
            QStringLiteral("zoom_wheel_bounce_latency_24"), 24, editor, paint_counter,
            [&](int step) {
            ...
            [&](Benchmark_editor& editor, int step) -> std::optional<Correctness_issue> {
                ...
                if (actual_zoom != expected_zoom) {
                    return Correctness_issue{
                        step, QStringLiteral("zoom mismatch: expected %1 got %2").arg(expected_zoom).arg(actual_zoom)};
                }
                return std::nullopt;
            }));
    }
