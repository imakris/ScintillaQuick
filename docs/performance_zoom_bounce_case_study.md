...
- That, in turn, is all in `update_from_visual_line`, 1680 calls -
  70 per paint x 24 paints. That lines up with "70 visible lines".
...
**Hypothesis (section 2.3 output):** on every zoom step, every visible line's
...
between exactly two zoom levels, much of the work being done after the
first visits to those two states is work that was already done for one
of the recent frames. If the node could remember one prior (`content`,
...
- For the zoom-bounce scenario in practice the per-run positions at
  ...
  In the measured run, the restore path handled the large majority of
  visual-line updates after the initial rebuilds.
...
`performance_optimization_process.md` section 2.2. Deltas smaller than the
...
- The dominant leaves (`add_text_layout`, `shape_text`) dropped by
  roughly the same 14x factor (55 ms -> 7 ms, 45 ms -> 4 ms), which is
...
The visual-regression target should be compared against the same
environment before and after the change. In the measured run for this
case study, the optimization did not introduce a distinct
visual-regression signature, but any future use of this document
should report the actual pre-change and post-change artifact
comparison rather than relying on this specific run.
## 7. Next Iteration Pointer

Per `performance_optimization_process.md` section 2.6, after a successful
