...
2. **A repeatable benchmark.** A deterministic workload that exercises the
   ...
   expected improvement is larger than the noise floor (see §2.2).
...
Compare the new distribution to the baseline distribution from §2.1 and
the noise bound from §2.2. An optimization is only "real" when:
...
- **Been eliminated** - it no longer shows up in the profile in any
  ...
  hotspot, and the loop continues from §2.3 with the new profile. This
...
- **Been reduced but is still dominant** - maybe another change can
  bite more of it. Return to §2.3 and re-run the drill-down on the
...
- `scintillaquick_embedded_benchmark` (run as a CTest entry, also
  launchable standalone) - doubles as a correctness check: every
  scenario's step function runs a verifier (`verify_visible_rows`,
  zoom level verification, etc.) and the benchmark returns exit code
  `2` if any verifier fails.
...
against a *real* `ScintillaQuickItem` instance inside a headless
...
- A nested `profiling` block containing the hierarchical profile (see
  §3.3) for the duration of that scenario.
...
same command 5+ times and compare the distribution (see
[`scripts/`](../scripts) if/when benchmark-automation helpers land).
...
The benchmark harness turns the profiler on for the exact duration of
each scenario's step function, so the profile attached to a scenario's
JSON is exactly the cost of that scenario's work and nothing else.

**Deepening the instrumentation.** When drill-down in §2.3 hits a leaf
...
anything when a profile session is active. This is what §2.3 calls
...
6. **Never skip §2.6.** After a successful iteration, re-profile and
...
- **Optimising on the leaf of the profile you happened to recognise.**
  The job of §2.3 is to let the profile pick the target, not your
  intuition.
